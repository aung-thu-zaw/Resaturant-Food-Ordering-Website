...
const props = defineProps({
  ...
  placeholder: {
    type: String,
    default: 'Filter By Author'
  }
})

const route = useRoute()
const router = useRouter()
const currentAuthor = ref(route.query.author ?? '')

watch(
  () => currentAuthor.value,
  (newAuthor) => {
    router.push({ query: { ...route.query, author: newAuthor } })
  }
)
...
watch(
  () => route.query,
  () => {
    if (!route.query.author) currentAuthor.value = ''
  }
)
</script>
