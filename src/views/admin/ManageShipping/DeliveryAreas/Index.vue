...
useTitle('Delivery Areas - Restaurant Food Ordering')

const route = useRoute()
const swal = inject('$swal')
const store = useDeliveryAreaStore()

const { deliveryAreas } = storeToRefs(store)
const { dashboardParams } = useQueryStringParams()

onMounted(async () => await store.getAllDeliveryAreas(dashboardParams.value))

const handleDeleteDeliveryArea = async (deliveryAreaSlug) => {
  ...
  if (result.isConfirmed) {
    await store.deleteDeliveryArea(deliveryAreaSlug)
  }
}

watch(
  () => route.query,
  async () => await store.getAllDeliveryAreas(dashboardParams.value)
)
</script>
...
<template>
  <DashboardLayout>
    ...
    <div class="min-h-screen py-10">
      ...
      >
        <Breadcrumb to="admin.dashboard" icon="fa-truck-fast" label="Delivery Areas">
      ...
      <div class="flex items-center justify-end mb-3">
        <!-- Create New Button -->
        <RouterLinkButton v-show="can('delivery-areas.create')" to="admin.delivery-areas.create">
      ...
      <div class="border border-gray-300 bg-white rounded-md shadow-sm shadow-gray-200 px-5 py-3">
        ...
        <TableContainer>
          <Table :items="deliveryAreas.data ?? []">
            <!-- Table Header -->
            ...
            <template #table-data="{ item }">
              ...
              <TableActionCell>
                <RouterLinkButton
                  v-show="can('delivery-areas.edit')"
                  to="admin.delivery-areas.edit"
                ...
                <NormalButton
                  v-show="can('delivery-areas.delete')"
                  @click="handleDeleteDeliveryArea(item.slug)"
        ...
        <Pagination :data="deliveryAreas" />

        <NoTableData v-show="!deliveryAreas?.data?.length" />
      </div>
      <!-- Table End -->
    </div>
  </DashboardLayout>
