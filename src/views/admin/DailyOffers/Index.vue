...
useTitle('Daily Offers - Restaurant Food Ordering')
...
onMounted(async () => await store.getAllDailyOffers(dashboardParams.value))

const handleDeleteDailyOffer = async (dailyOfferId) => {
  ...
  if (result.isConfirmed) {
    await store.deleteDailyOffer(dailyOfferId)
  }
}

watch(
  () => route.query,
  async () => await store.getAllDailyOffers(dashboardParams.value)
)
</script>
...
<template>
  <DashboardLayout>
    ...
    <div class="min-h-screen py-10">
      ...
      >
        <Breadcrumb to="admin.dashboard" icon="fa-clock" label="Daily Offers">
      ...
      <div class="flex items-center justify-end mb-3">
        <!-- Create New Button -->
        <RouterLinkButton v-show="can('daily-offers.create')" to="admin.daily-offers.create">
          <i class="fa-solid fa-file-circle-plus mr-1"></i>
          Add A New Daily Offer
        </RouterLinkButton>
      </div>
      ...
      <div class="border border-gray-300 bg-white rounded-md shadow-sm shadow-gray-200 px-5 py-3">
        ...
        <TableContainer>
          <Table :items="dailyOffers.data ?? []">
            ...
            <template #table-header>
              ...
              <SortableTableHeaderCell label="Percentage" sort="discount_percentage" />
            ...
            <template #table-data="{ item }">
              ...
              <TableDataCell class="min-w-[150px]">
                {{ formatAmount(item?.discount_percentage) }} %
              </TableDataCell>

              <TableActionCell>
                <RouterLinkButton
                  v-show="can('daily-offers.edit')"
                  to="admin.daily-offers.edit"
                ...
                <NormalButton
                  v-show="can('daily-offers.delete')"
                  @click="handleDeleteDailyOffer(item.id)"
        ...
        <Pagination :data="dailyOffers" />

        <NoTableData v-show="!dailyOffers?.data?.length" />
      </div>
      <!-- Table End -->
    </div>
  </DashboardLayout>
