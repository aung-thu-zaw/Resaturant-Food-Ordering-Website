...
useTitle('Edit Daily Offer - Restaurant Food Ordering')
...
onMounted(async () => {
  await store.getDailyOffer(props.id)

  await store.getResources()

  form.product_id = store.dailyOffer?.product_id
  form.start_date = store.dailyOffer?.start_date
  form.end_date = store.dailyOffer?.end_date
  form.discount_percentage = store.dailyOffer?.discount_percentage
})

const handleUpdateDailyOffer = async () => await store.updateDailyOffer({ ...form }, props.id)
</script>
  ...
  <DashboardLayout>
    <div class="min-h-screen py-10">
      ...
      <div class="border p-10 bg-white rounded-md">
        <form @submit.prevent="handleUpdateDailyOffer" class="space-y-4 md:space-y-6">
          <div>
