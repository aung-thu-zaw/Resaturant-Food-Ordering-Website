...
useTitle('Blog Categories - Restaurant Food Ordering')
...
onMounted(async () => await store.getAllBlogCategories(dashboardParams.value))

const handleDeleteBlogCategory = async (blogCategorySlug) => {
  ...
  if (result.isConfirmed) {
    await store.deleteBlogCategory(blogCategorySlug)
  }
}

watch(
  () => route.query,
  async () => await store.getAllBlogCategories(dashboardParams.value)
)
</script>
...
<template>
  <DashboardLayout>
    ...
    <div class="min-h-screen py-10">
      ...
      >
        <Breadcrumb to="admin.dashboard" icon="fa-pen-nib" label="Blog Categories">
      ...
      <div class="flex items-center justify-end mb-3">
        <!-- Create New Button -->
        <RouterLinkButton v-show="can('blog-categories.create')" to="admin.blog-categories.create">
      ...
      <div class="border border-gray-300 bg-white rounded-md shadow-sm shadow-gray-200 px-5 py-3">
        ...
        >
          <DashboardTableDataSearchBox placeholder="Search by category name" />
            ...
            <template #table-data="{ item }">
              ...
              <TableActionCell>
                ...
                <NormalButton
                  ...
                  @click="handleDeleteBlogCategory(item.slug)"
