...
useTitle('Edit Blog Category - Restaurant Food Ordering')
...
onMounted(async () => {
  await store.getBlogCategory(props.slug)

  form.name = store.blogCategory?.name
})

const handleUpdateBlogCategory = async () => await store.updateBlogCategory({ ...form }, props.slug)
</script>
  ...
  <DashboardLayout>
    <div class="min-h-screen py-10">
      ...
      <div class="border p-10 bg-white rounded-md">
        <form @submit.prevent="handleUpdateBlogCategory" class="space-y-4 md:space-y-6">
          <div>
